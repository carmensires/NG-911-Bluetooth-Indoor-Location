...
public class Call implements InviteDialogListener {
    ...
    /*
     * Creates an ExtendedInviteDialog
     *
     * passes the received location as a parameter to inviteNG911
     */
    /*
    public void callNG911(String callee, String from, String contact, String sdp, String icsi) {
        printLog("calling " + callee, LogLevel.HIGH);
        if (from == null)
            from = from_url;
        if (contact == null)
        ...
        dialog = new InviteDialog(sip_provider, this);
        if (local_sdp != null)
            dialog.inviteNG911(callee, from, contact, local_sdp, icsi);
        else
            dialog.inviteNG911(callee, from, contact, null, null);
    }
    */
