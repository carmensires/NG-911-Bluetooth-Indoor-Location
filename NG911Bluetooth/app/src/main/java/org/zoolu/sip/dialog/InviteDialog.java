...
import com.android.albert.ng911.Data;
import com.android.albert.ng911.HttpTx;
...
public class InviteDialog extends Dialog implements TransactionClientListener,
     ...
     * 1. Receives the indoor location from the server and puts this information into a SIP Invite message to the ESInet
     * 2. In case of an error on the location server, it will send an INVITE to the ESInet
     * but the INVITE will have an empty location.
     */
    public void inviteNG911(String callee, String caller, String contact,
                            String session_descriptor, String icsi) {
        printLog("inside invite(callee,caller,contact,sdp)", LogLevel.MEDIUM);

        if (session_descriptor == null) {
            invite_offer = false;
        }
        ...
        if (!statusIs(D_INIT))
            return;
        // else
        final NameAddress to_url = new NameAddress(callee);//to_global = new NameAddress(callee);     //we declare them as 'final'
        final NameAddress from_url = new NameAddress(caller);//from_global = new NameAddress(caller);   //to let them be used
        final SipURL request_uri = to_url.getAddress();//request_uri_global = to_global.getAddress();         //after the HTTP request is successful
        ...
        if (contact != null) {
            ...
            contact_url = from_url;//from_global;

        // 0. Get Call ID, in order to send it to location provider
        call_id = sip_provider.pickCallId();

        //necessary final local variables used after the HTTP request is responded
        final NameAddress contact_url_local = contact_url;
        final String sdp_local = session_descriptor;
