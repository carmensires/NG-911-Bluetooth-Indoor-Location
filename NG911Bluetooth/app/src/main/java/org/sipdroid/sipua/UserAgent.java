...
//	private void addMediaDescriptor(String media, int port, Codecs.Map c,int rate) {
    ...
    public boolean call(String target_url, boolean send_anonymous, boolean ng911) { //NG911 boolean added
        ...
        if (ng911) { //change request URI in case of an emergency call

            target_url = "urn:service:sos";

            android.util.Log.d("NG911 [UserAgent]", "Request URI and 'From: ' header field have been generated");
            android.util.Log.d("NG911 [UserAgent]", "Request URI: " + target_url);
            android.util.Log.d("NG911 [UserAgent]", "'From: ' header field: " + from_url);

            //Go to ExtendedCall.java and execute callNG911(...)
            if (user_profile.no_offer) {
                call.callNG911(target_url, null, null, null, null);
            } else {
                call.callNG911(target_url, null, null, local_session, icsi);
            }
